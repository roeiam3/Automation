...
- Phase 1: interface bring-up + MLAG configuration + LLDP checks
- Phase 2: loopbacks
...
- MLAG peerlink ports (hardcoded by design): `swp1-2` for all MLAG pairs.
- Server-facing connectivity is kept on low-end ports by design.
  ...
  - server-side: bonded across two leafs in same MLAG pair via `eth1` + `eth2`
...
- Server-facing leaf bond interface: `leaf_server_bond_name`
...
- MLAG pair ID derived from leaf hostname index: `ceil(leaf_index / 2)`.
- MLAG system MAC derived from pair ID: `44:38:39:be:ef:XX` (hex of pair ID).
- MLAG backup IP pulled from inventory hostvars of peer leaf.
- MLAG peerlink hardset to `swp1-2` (documented port contract).
- Leaf-side server-facing bond uses `leaf_server_bond_name` on `leaf_server_port` with MLAG enabled.
